...
type
  ...
  TShowHintCellProc = procedure (Sender:TObject; col,row:longint; var HintStr: string;
    var CanShow: Boolean; var HintInfo: Controls.THintInfo) of object;
...
(*$ifdef delphi_ge_3 *)
  ...
  (*@\\\0000000401*)
(*$endif *)
  TCompareFunction = function(Sender:TObject; colrow,compare1,compare2:longint; Data:string):t_relation of object;
  TSortFunction = procedure (ColRow,Min,Max: longint; ByColumn,ascending: boolean; Data:string) of object;
  ...
  protected
    ...
    FAutoSortColumns : Boolean;
    FLastColClick : Integer;
    FLastSortType : Boolean;
  ...
  private
    function GetCells(ACol: String; ARow: Integer): string;
    procedure SetCells(ACol: String; ARow: Integer; const Value: string);
  protected
  ...
  public
    property CellsName[ACol:String; ARow: Integer] : string read GetCells write SetCells;
  protected
    FAlign: TMyAlign;
  ...
  public
    ...
    (*@\\\0000000801*)
  published
    property ColNames : TStringList read FColNames write FColNames;
  ...
  published
    ...
    property AutoSortColumns : Boolean read FAutoSortColumns write FAutoSortColumns default false;
  ...
  public
    ...
    procedure Vaciar; overload;
    procedure Vaciar(p_boBorrarHints, p_boBorrarObjs : Boolean; pniFilaDesde : Integer = -1; pniCantidadFilas : Integer = 2); overload;
    procedure Limpiar; overload;
    procedure Limpiar(p_boBorrarHints, p_boBorrarObjs : Boolean; pniFilaDesde : integer = -1); overload;
    procedure LimpiarFila(pniFila : integer = -1); overload;
    procedure LimpiarFila(p_boBorrarHints, p_boBorrarObjs : Boolean; pniFila : integer); overload;
  ...
  protected
    ...
    procedure DoSortBubble(ColRow,Min,Max: longint; ByColumn,ascending:boolean; Data:string);
    procedure DoSortQuick(ColRow,Min,Max: longint; ByColumn,ascending:boolean; Data:string);
  public
    function CompareColString(Sender: TObject; Column, Row1,Row2: longint; Data:string):t_relation;
    function CompareRowString(Sender: TObject; RowNr, Col1,Col2: longint; Data:string):t_relation;
    ...
    procedure SortColumn(Column: longint; Ascending:boolean; Data: String);
    procedure SortRow(Rownumber: longint; Ascending:boolean; Data: string);
  published
...
{                               (* but makes it behave erratically *)       }
    or Dword(Alignments[FAlignment]);
  end;
(*@\\\0000000B01*)
...
procedure TStringAlignGrid.Vaciar;
begin
  Vaciar(false, false);
end;

procedure TStringAlignGrid.Vaciar(p_boBorrarHints, p_boBorrarObjs : Boolean; pniFilaDesde : Integer = -1; pniCantidadFilas : Integer = 2);
begin
  BeginUpdate;
  Limpiar(p_boBorrarHints, p_boBorrarObjs, pniFilaDesde);

  if FixedRows+1 > pniCantidadFilas then
    RowCount := FixedRows+1
  else
    RowCount := pniCantidadFilas;

  EndUpdate;
end;

procedure TStringAlignGrid.Limpiar;
begin
  Limpiar(false, false);
end;

procedure TStringAlignGrid.Limpiar(p_boBorrarHints, p_boBorrarObjs : Boolean; pniFilaDesde : integer = -1);
var
  I, J : Integer;
begin
  BeginUpdate;

  if pniFilaDesde = -1 then
    pniFilaDesde := FixedRows;

  ResetAllCellAll;

  for I := pniFilaDesde to RowCount-1 do
  begin
    // esto borra el texto de todas las celdas de la fila
    Rows[I].CommaText := '';

    if (p_boBorrarHints) or (p_boBorrarObjs) then
    begin
    for J:=0 to ColCount-1 do
    begin
      if p_boBorrarHints then
        HintCell[J,I]:='';

      if p_boBorrarObjs then
    ...
    RowFont[I] := Self.Font;
    ColorRow[I] := Self.Color;
  end;

  EndUpdate;
end;

procedure TStringAlignGrid.LimpiarFila(pniFila : integer = -1);
begin
  if pniFila = -1 then
    pniFila := Row;

  LimpiarFila(false, false, pniFila);
end;

procedure TStringAlignGrid.LimpiarFila(p_boBorrarHints, p_boBorrarObjs : Boolean; pniFila : integer);
var
  J : Integer;
begin
  BeginUpdate;
  ...
  begin
    ...
    if p_boBorrarObjs then
      if Assigned(Objects[J,pniFila]) then
      begin
        Objects[J, pniFila].Free;
        Objects[J, pniFila] := nil;
      end;
  end;
  end;

  RowFont[pniFila] := Self.Font;
  ColorRow[pniFila] := Self.Color;
...
function TStringAlignGrid.GetCells(ACol: String; ARow: Integer): string;
var
  col : integer;
begin
  if not TryStrToInt(ColNames.Values[ACol],col) then
    col := -1;
  result := Cells[col,aRow];
end;
...
procedure TStringAlignGrid.SetCells(ACol: String; ARow: Integer; const Value: string);
var
  col : integer;
begin
  if not TryStrToInt(ColNames.Values[ACol],col) then
    col := -1;
  Cells[col,aRow] := Value;
end;
...
begin
  if not (csDesigning in ComponentState) then
    Self.Hint := 'Total de Registros: ' + IntToStr(Self.RowCount-1);
...
begin
  ...
  while not Reader.EndOfList do begin
    ACol:=Reader.ReadInteger;
    ARow:=Reader.ReadInteger;
    v:=NewStr(AnsiString(Reader.ReadString));
    v:=SetItemCell(ACol,ARow, FHintCell, v);
    FSaveHint:=true;
    if v<>NIL then
      DisposeStr(v);
  ...
  for ACol:=0 to ColCount-1 do
    for ARow:=0 to RowCount-1 do begin
      if cells[ACol,ARow]<>'' then begin
        v:=NewStr(AnsiString(cells[ACol,ARow]));
        end
      else
        v:=NIL;
...
procedure TStringAlignGrid.DoSortBubble(ColRow,Min,Max: longint; ByColumn,ascending: boolean; Data:string);
var
  i,j,k: longint;
begin
  for i:=Min to Max-1 do begin
    ...
    for j:=i+1 to Max do begin
      if ByColumn then
        case f_compare_col(self,ColRow,k,j,'S') of
          rel_equal: ;
          rel_greater: if ascending then k:=j;
          rel_less:    if not ascending then k:=j;
          end
      else
        case f_compare_row(self,ColRow,k,j,'S') of
          rel_equal: ;
          rel_greater: if ascending then k:=j;
          rel_less:    if not ascending then k:=j;
          end;
...
procedure TStringAlignGrid.DoSortQuick(ColRow,Min,Max: longint; ByColumn,ascending: boolean; Data:string);
...
begin
  ...
  while true do begin
    ...
    while aLessThan(self,colrow,i,R,Data)=comp do
      inc(i);
    while aLessThan(self,colrow,R,j,Data)=comp do begin
      if (j = L) then
        BREAK;
      dec(j);
      end;
...
function TStringAlignGrid.CompareColString(Sender: TObject; Column, Row1,Row2: longint; Data:string):t_relation;
var num1,num2:Double;
    cmonto:string;
begin
   if ((Data='N') or (Data='n')) then
   begin
     if (Copy(Cells[Column,Row1],1,1)='(')then
     begin
       cmonto:=copy(trim(Cells[Column, Row1]),2,9);
       cmonto:=copy(cmonto,1,length(cmonto)-1);
       try
         num1:=StrtoFLoat(cmonto)*-1;
       except
         num1:=0;
       end;
     end
     ...
     begin
       try
         num1:=StrToFloat(Cells[Column,Row1]);
       except
         num1:=0;
       end;
     end;

     if (Copy(Cells[Column,Row2],1,1)='(')then
     begin
       cmonto:=copy(trim(Cells[Column, Row2]),2,9);
       cmonto:=copy(cmonto,1,length(cmonto)-1);
       try
         num2:=StrtoFLoat(cmonto)*-1;
       except
         num2:=0;
       end;
     end
     ...
     begin
       try
         num2:=StrToFloat(Cells[Column,Row2]);
       except
         num2:=0;
       end;
     end;
   ...
   else if ((Data='D') or (Data='d')) then
   begin
     if false then
     else if concat(Copy(Cells[Column,Row1],7,4),Copy(Cells[Column,Row1],4,2),Copy(Cells[Column,Row1],1,2))=concat(Copy(Cells[Column,Row2],7,4),Copy(Cells[Column,Row2],4,2),Copy(Cells[Column,Row2],1,2)) then
       result:=rel_equal
     else if concat(Copy(Cells[Column,Row1],7,4),Copy(Cells[Column,Row1],4,2),Copy(Cells[Column,Row1],1,2))>concat(Copy(Cells[Column,Row2],7,4),Copy(Cells[Column,Row2],4,2),Copy(Cells[Column,Row2],1,2)) then
       result:=rel_greater
     else
   ...
   else if ((Data='T') or (Data='t')) then
   begin
     if false then
     else if concat(Copy(Cells[Column,Row1],7,4),Copy(Cells[Column,Row1],4,2),Copy(Cells[Column,Row1],1,2),Copy(Cells[Column,Row1],12,5))=concat(Copy(Cells[Column,Row2],7,4),Copy(Cells[Column,Row2],4,2),Copy(Cells[Column,Row2],1,2),Copy(Cells[Column,Row2],12,5)) then
       result:=rel_equal
     else if concat(Copy(Cells[Column,Row1],7,4),Copy(Cells[Column,Row1],4,2),Copy(Cells[Column,Row1],1,2),Copy(Cells[Column,Row1],12,5))>concat(Copy(Cells[Column,Row2],7,4),Copy(Cells[Column,Row2],4,2),Copy(Cells[Column,Row2],1,2),Copy(Cells[Column,Row2],12,5)) then
       result:=rel_greater
     else
   ...
   else //  if ((Data='S') or (Data='s')) then
   begin
     if Cells[Column,Row1]=Cells[Column,Row2] then
       result:=rel_equal
     else
     begin
       if Cells[Column,Row1]>Cells[Column,Row2] then
...
function TStringAlignGrid.CompareRowString(Sender: TObject; RowNr, Col1,Col2: longint; Data:string):t_relation;
begin
  if false then
...
procedure TStringAlignGrid.SortColumn(Column: longint; Ascending:boolean; Data: String);
//var
//  Filas : Integer;
begin
  if not assigned(f_compare_col) then EXIT;
  if ((Data<>'S') and (Data<>'s') and (Data<>'N') and (Data<>'n') and (Data<>'D') and (Data<>'d') and (Data<>'T') and (Data<>'t')) then Exit;
//  Filas := RowCount;
//  RowCount := 2;
  fSortMethod(Column,FixedRows,RowCount-1,true,Ascending,Data);
  //RowCount := Filas;
  end;
(*@\\\000000040E*)
(*@/// procedure TStringAlignGrid.SortRow(RowNumber: longint; Ascending:boolean); *)
procedure TStringAlignGrid.SortRow(RowNumber: longint; Ascending:boolean; Data: string);
begin
  if not assigned(f_compare_row) then EXIT;
  fSortMethod(RowNumber,FixedCols,ColCount-1,false,Ascending,Data);
  end;
(*@\\\0000000401*)
(*@\\\0000000301*)
(*@/// Miscellaneous stuff *)
...
procedure TStringAlignGrid.WMChar(var Msg: TWMChar);
begin
  if (goEditing in Options) and CharInSet(Char(Msg.CharCode), [^H, #32..#255]) then begin
...
begin
  if ((x>=0) or (y>=0)) and (button=mbLeft) then begin
    ...
    else if (ARow<FixedRows) and (GetCursor <> Screen.Cursors[crHSplit]) then
    begin
      if assigned(f_fixedcolclick) then
        f_fixedcolclick(Self,ACol)
      else if FAutoSortColumns then
      begin
        if FLastColClick = ACol then
          FLastSortType := not FLastSortType
        else
        begin
          FLastColClick := ACol;
          FLastSortType := true;
        ...
        SortColumn(ACol, FLastSortType, 'S');
      end;
    end;
...
procedure TStringAlignGrid.BeginUpdate;
begin
  if (not Self.Visible) or (not Assigned(Self.Owner)) then
    Exit;

  inc(F_ContadorUpdates);
  if (F_ContadorUpdates = 1) then
    SendMessage(self.Handle, WM_SETREDRAW, 0, 0);
end;

procedure TStringAlignGrid.EndUpdate;
begin
  if (F_ContadorUpdates > 0) then
    dec(F_ContadorUpdates);
...
procedure TStringAlignGrid.ResetUpdates;
begin
  f_ContadorUpdates := 0;
  EndUpdate;
end;
