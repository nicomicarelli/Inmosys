...
functions :
  ...
  HtmlHelpShowTopic(const aTopic : AnsiString) : HWND
...
const
   ...
   HH_DISPLAY_TEXT_POPUP    = $000E;  // display AnsiString resource id or text in a popup window
...
type
   ...
   HHN_NOTIFY = record
      hdr : NMHDR;
      pszUrl : PWideChar;        // Multi-byte, null-terminated AnsiString
   end;
   THHNNotiy = HHN_NOTIFY;

   HH_POPUP = record
      cbStruct : Integer;        // sizeof this structure
      hinst : Integer;           // instance handle for AnsiString resource
      idAnsiString : Integer;        // AnsiString resource id, or text id if pszFile is specified in HtmlHelp call
      pszText : pAnsiChar;           // used if idAnsiString is zero
...
type
   ...
   HH_FTS_QUERY = record
      ...
      fUniCodeAnsiStrings : BOOL;    // TRUE if all AnsiStrings are unicode.
      pszSearchQuery : pAnsiChar;    // AnsiString containing the search query.
   ...
   HH_WINTYPE = record
      ...
      fUniCodeAnsiStrings : BOOL; // IN/OUT: TRUE if all AnsiStrings are in UNICODE
      ...
      pszCustomTabs : pAnsiChar;  // multiple zero-terminated AnsiStrings
   end;
...
type
   ...
   HHNTRACK = record
      hdr : NMHDR;
      pszCurUrl : pAnsiChar;         // Multi-byte, null-terminated AnsiString
      idAction : Integer;        // HHACT_ value
      phhWinTyp : PHH_WINTYPE;   // Current window type structure
   end;

   function HtmlHelp(hwndCaller : HWND; pszFile: pAnsiChar; uCommand : Integer;
...
function HtmlHelpShowTopic(const aTopic : AnsiString) : HWND;
...
procedure HtmlHelpRutaAyuda( const Ruta : String );
{$ENDIF}

var
  ArchivoAyuda : AnsiString;
...
function HtmlHelpShowHelp : HWND;
begin
  Result:=HtmlHelp(0, pAnsiChar(ArchivoAyuda), HH_DISPLAY_TOPIC, 0);
end;

function HtmlHelpShowTopic(const aTopic : AnsiString) : HWND;
begin
  Result:=HtmlHelp(0, pAnsiChar(ArchivoAyuda+'::\'+aTopic), HH_DISPLAY_TOPIC, 0);
end;

function HtmlHelpShowContents : HWND;
begin
  Result:=HtmlHelp(0, pAnsiChar(ArchivoAyuda), HH_DISPLAY_TOC, 0);
end;

function HtmlHelpShowIndex : HWND;
begin
  Result:=HtmlHelp(0, pAnsiChar(ArchivoAyuda), HH_DISPLAY_INDEX, 0);
end;

function HtmlHelpShowContext( const aContext : Integer ) : HWND;
begin
  Result:=HtmlHelp(0, pAnsiChar(ArchivoAyuda), HH_HELP_CONTEXT, aContext );
end;

procedure HtmlHelpRutaAyuda( const Ruta : String );
begin
  ArchivoAyuda := AnsiString(Ruta);
end;
