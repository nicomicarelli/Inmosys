...
type
  TfAbmRazonesSociales = class(TForm)
...
var
  fAbmRazonesSociales: TfAbmRazonesSociales;
...
procedure TfAbmRazonesSociales.ActualizarClick(Sender: TObject);
var
  q: TFXQuery;
  I: Integer;
begin
  q := CrearQuery;
  try
    q.SQL.Text := 'Select * from RazonesSociales where 1=1';
    ...
    begin
      ...
      gDetalle.ColorCell[0,I] := StringToColor(q.FieldByName('Color').AsString);
      Inc(I);
      q.Next;
    end;
    if i>2 then
...
function TfAbmRazonesSociales.TituloGetFieldValue(aFieldName: string): Variant;
begin
...
procedure TfAbmRazonesSociales.TodosCodigosClick(Sender: TObject);
begin
  if todosCodigos.Checked then
  begin
    edDesde.Text:='';
...
procedure TfAbmRazonesSociales.TodosDescripcionClick(Sender: TObject);
begin
  if todosDescripcion.Checked then
    edDescripcion.Text:='';
...
procedure TfAbmRazonesSociales.btnEliminarClick(Sender: TObject);
begin
  if gDetalle.Cells[0,gDetalle.Row] = '' then
    Exit;
  if FRazonesSociales = nil then
    Application.CreateForm(TFRazonesSociales, fRazonesSociales)
  else
    FRazonesSociales.Show;

  FRazonesSociales.Cargar(gDetalle.Cells[0,gDetalle.Row],toEliminar);

end;

procedure TfAbmRazonesSociales.btnImprimirClick(Sender: TObject);
begin
...
procedure TfAbmRazonesSociales.btnModificarClick(Sender: TObject);
begin
  if gDetalle.Cells[0,gDetalle.Row] = '' then
    Exit;
  ...
  FRazonesSociales.Cargar(gDetalle.Cells[0,gDetalle.Row],toModificar);
end;

procedure TfAbmRazonesSociales.btnNuevoClick(Sender: TObject);
begin
  if FRazonesSociaLES = nil then
    Application.CreateForm(TFRazonesSociales, FRazonesSociales)
  else
    FRazonesSociales.Show;
  FRazonesSociales.Cargar('',toNuevo);
end;

procedure TfAbmRazonesSociales.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TfAbmRazonesSociales.edDescripcionChange(Sender: TObject);
begin
...
procedure TfAbmRazonesSociales.edDesdeChange(Sender: TObject);
begin
...
procedure TfAbmRazonesSociales.edDesdeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  edHasta.Text := edDesde.text;
end;
...
procedure TfAbmRazonesSociales.edDesdeKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfAbmRazonesSociales.edHastaChange(Sender: TObject);
begin
...
procedure TfAbmRazonesSociales.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action := caFree;
  fAbmRazonesSociales := nil;
end;

procedure TfAbmRazonesSociales.FormCreate(Sender: TObject);
begin
  fPrincipal.Center(Self);
end;

procedure TfAbmRazonesSociales.gDetalleDblClick(Sender: TObject);
begin
...
procedure TfAbmRazonesSociales.gDetalleFixedColClick(Sender: TObject; col: Integer);
begin
