...
var
  fABMIndicadores: TfABMIndicadores;

implementation

uses frmprincipal, frmNuevaCaja, frmNuevoIndicador;

{$R *.dfm}

procedure TfABMIndicadores.btnActualizarClick(Sender: TObject);
var
  q: TFXQuery;
begin
  q := CrearQuery;
  try
    q.SQL.Text := 'Select * from Indicadores where 1=1';
...
procedure TfABMIndicadores.btnEliminarClick(Sender: TObject);
begin
  if FNuevoIndicador = nil then
    Application.CreateForm(TFNuevoIndicador, FNuevoIndicador)
  else
    FNuevoIndicador.Show;

  FNuevoIndicador.Cargar(cdsDatos.FieldbyName('Codigo').asInteger, toEliminar);
end;

procedure TfABMIndicadores.btnImprimirClick(Sender: TObject);
var
...
procedure TfABMIndicadores.btnModificarClick(Sender: TObject);
begin
  if FNuevoIndicador = nil then
    Application.CreateForm(TFNuevoIndicador, FNuevoIndicador)
  else
  ...
  FNuevoIndicador.Cargar(cdsDatos.FieldbyName('Codigo').asInteger, toModificar);
end;

procedure TfABMIndicadores.btnNuevoClick(Sender: TObject);
begin
  if FNuevoIndicador = nil then
    Application.CreateForm(TFNuevoIndicador, FNuevoIndicador)
  else
  ...
  FNuevoIndicador.Cargar(0, toNuevo);
end;

procedure TfABMIndicadores.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TfABMIndicadores.edDescripcionChange(Sender: TObject);
begin
...
procedure TfABMIndicadores.edDesdeChange(Sender: TObject);
begin
...
procedure TfABMIndicadores.edHastaChange(Sender: TObject);
begin
...
procedure TfABMIndicadores.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  fABMIndicadores := nil;
  Action := caFree;
end;

procedure TfABMIndicadores.FormCreate(Sender: TObject);
begin
  fPrincipal.Center(Self);
  prPrepararDataset;
end;

procedure TfABMIndicadores.prPrepararDataset;
begin
...
function TfABMIndicadores.TituloGetFieldValue(aFieldName: string): Variant;
begin
...
procedure TfABMIndicadores.TodosCodigosClick(Sender: TObject);
begin
  if todosCodigos.Checked then
  begin
    edDesde.Text:='';
...
procedure TfABMIndicadores.TodosDescripcionClick(Sender: TObject);
begin
  if todosDescripcion.Checked then
    edDescripcion.Text:='';
