unit frmHistorialEntregaLiquidaciones;
...
type
  TfHistorialEntregaLiquidaciones = class(TForm)
...
var
  fHistorialEntregaLiquidaciones: TfHistorialEntregaLiquidaciones;
...
procedure TfHistorialEntregaLiquidaciones.ActualizarClick(Sender: TObject);
var
  q: TFXQuery;
...
begin
  ...
  try
    ...
    q.SQL.Add(' order by I.Propietario, CA.Tipo, Ca.Numero, Ca.Fecha ');
...
procedure TfHistorialEntregaLiquidaciones.TodosFPagoClick(Sender: TObject);
begin
...
function TfHistorialEntregaLiquidaciones.TituloGetFieldValue(
...
procedure TfHistorialEntregaLiquidaciones.TodosCodigosClick(Sender: TObject);
begin
  if todosCodigos.Checked then
  begin
    edDesde.Text:='';
...
procedure TfHistorialEntregaLiquidaciones.VerContratoEscalonado1Click(Sender: TObject);
var
  Tecla: Char;
begin
...
procedure TfHistorialEntregaLiquidaciones.btnEliminarClick(Sender: TObject);
begin
...
procedure TfHistorialEntregaLiquidaciones.btnGrabarClick(Sender: TObject);
begin
    ImprimirReporte ( Reporte,  plReporte,  gDetalle ) ;
end;

procedure TfHistorialEntregaLiquidaciones.btnModificarClick(Sender: TObject);
begin
  if gDetalle.Cells[6,gDetalle.Row] = '' then
    Exit;
...
procedure TfHistorialEntregaLiquidaciones.btnNuevoClick(Sender: TObject);
begin
  if FMovimientosCtaCte = nil then
    Application.CreateForm(TFMovimientosCtaCte, FMovimientosCtaCte)
  else
...
procedure TfHistorialEntregaLiquidaciones.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TfHistorialEntregaLiquidaciones.cbFechaPagoClick(Sender: TObject);
begin
  todosFPago.Checked := False;
end;

procedure TfHistorialEntregaLiquidaciones.edDescripcionKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
    edtFecha.SetFocus;
...
procedure TfHistorialEntregaLiquidaciones.edDesdeChange(Sender: TObject);
begin
...
procedure TfHistorialEntregaLiquidaciones.edDesdeKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfHistorialEntregaLiquidaciones.edFechaHastaChange(Sender: TObject);
begin
//  TodosFPago.checked := Trim(edFechaHasta.Text) = '';
end;

procedure TfHistorialEntregaLiquidaciones.edFechaHastaKeyPress(Sender: TObject; var Key: Char);
var
  Dia:   string;
  Fecha: string;
...
procedure TfHistorialEntregaLiquidaciones.edHastaChange(Sender: TObject);
begin
...
procedure TfHistorialEntregaLiquidaciones.edtFechaChange(Sender: TObject);
begin
  TodosFPago.checked := Trim(edtFecha.Text) = '';
//  edFechahasta.Text:=edtfecha.Text;
end;

procedure TfHistorialEntregaLiquidaciones.edtFechaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  edFechaHasta.Text := edtfecha.text;
end;

procedure TfHistorialEntregaLiquidaciones.edtFechaKeyPress(Sender: TObject; var Key: Char);
var
  Dia:   string;
  Fecha: string;
...
procedure TfHistorialEntregaLiquidaciones.edtItemDesdeChange(Sender: TObject);
begin
//  edItemhasta.Text:=edtItemdesde.Text;

end;

procedure TfHistorialEntregaLiquidaciones.edtItemDesdeKeyPress(Sender: TObject; var Key: Char);
begin
  if  not (CharInSet(Key, ['0'..'9', #8, #13])) then
    Key := #0;
...
procedure TfHistorialEntregaLiquidaciones.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action := caFree;
  fHistorialEntregaLiquidaciones := nil;
end;

procedure TfHistorialEntregaLiquidaciones.FormCreate(Sender: TObject);
begin
  fPrincipal.Center(Self);
end;

procedure TfHistorialEntregaLiquidaciones.gDetalleDblClick(Sender: TObject);
begin
...
procedure TfHistorialEntregaLiquidaciones.gDetalleFixedColClick(Sender: TObject; col: Integer);
begin
