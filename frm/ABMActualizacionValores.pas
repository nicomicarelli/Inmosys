...
uses
  ...
  ppDesignLayer, fxQuerY, Data.DB, cxGroupBox, AdvPanel, dbClient, cxStyles,
  dxSkinscxPCPainter, cxCustomData, cxFilter, cxData, cxDataStorage,
  cxNavigator, cxDBData, cxGridLevel, cxGridCustomTableView, cxGridTableView,
  cxGridDBTableView, cxClasses, cxGridCustomView, cxGrid, uImpresion, cxSpinEdit;

type
  TfABMActualizacionValores = class(TForm)
...
var
  fABMActualizacionValores: TfABMActualizacionValores;

implementation

uses frmDatosFijos, frmPrincipal,
  frmNuevoImpuesto, ABMImpuestosYServicios, frmNuevoCedulon,
  frmActualizacionValores;

{$R *.dfm}

procedure TfABMActualizacionValores.ActualizarClick(Sender: TObject);
var
  q: TFXQuery;
begin
...
function TfABMActualizacionValores.TituloGetFieldValue(aFieldName: string): Variant;
begin
...
procedure TfABMActualizacionValores.TodosCodigosClick(Sender: TObject);
begin
  if todosCodigos.Checked then
  begin
    edDesde.Text:='';
...
procedure TfABMActualizacionValores.TodosDescripcionClick(Sender: TObject);
begin
  if todosDescripcion.Checked then
    edDescripcion.Text:='';
...
procedure TfABMActualizacionValores.btnEliminarClick(Sender: TObject);
begin
...
procedure TfABMActualizacionValores.btnImprimirClick(Sender: TObject);
var
...
procedure TfABMActualizacionValores.btnModificarClick(Sender: TObject);
begin
  if cdsDatos.FieldByName('Codigo').AsString = '' then
    Exit;
...
procedure TfABMActualizacionValores.btnNuevoClick(Sender: TObject);
begin
  if fActualizacionValores = nil then
    Application.CreateForm(TfActualizacionValores, fActualizacionValores)
  else
    fActualizacionValores.Show;
  fActualizacionValores.edCodigo.Text := cdsDatos.FieldByName('Codigo').AsString;
  fActualizacionValores.edEmpresa.Text := cdsDatos.FieldByName('Empresa').AsString;
  fActualizacionValores.edMes.Text := cdsDatos.FieldByName('Mes').AsString;
  fActualizacionValores.spAnio.Text := cdsDatos.FieldByName('Anio').AsString;

  fActualizacionValores.ActualizarClick(nil);
end;

procedure TfABMActualizacionValores.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TfABMActualizacionValores.edDescripcionChange(Sender: TObject);
begin
...
procedure TfABMActualizacionValores.edDesdeChange(Sender: TObject);
begin
...
procedure TfABMActualizacionValores.edDesdeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  edHasta.Text := edDesde.text;
end;
...
procedure TfABMActualizacionValores.edDesdeKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfABMActualizacionValores.edHastaChange(Sender: TObject);
begin
...
procedure TfABMActualizacionValores.fAsignacionBoletasNuevoCopiadelSeleccionado1Click(
  Sender: TObject);
begin
  if cdsDatos.FieldByName('Codigo').AsString = '' then
    Exit;
...
procedure TfABMActualizacionValores.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action := caFree;
  fABMActualizacionValores := nil;
end;

procedure TfABMActualizacionValores.FormCreate(Sender: TObject);
begin
...
procedure TfABMActualizacionValores.gDetalleFixedColClick(Sender: TObject; col: Integer);
begin
//  gDetalle.SortColumn(Col, True,'S');
end;

procedure TfABMActualizacionValores.prPonerTitulosGrilla(pGrilla: TAdvStringGrid);
var
...
function TfABMActualizacionValores.fxValidarPorcentajesPago: Boolean;
var
  q: TFXQuery;
begin
  Result := True;
...
procedure TfABMActualizacionValores.prPrepararDataset;
var
  I: Integer;
