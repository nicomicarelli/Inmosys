...
type
  TfABMCategoriasVales = class(TForm)
...
var
  fABMCategoriasVales: TfABMCategoriasVales;

implementation

uses frmprincipal, frmNuevaCatetgoriaVale, frmNuevoIndicador;

{$R *.dfm}

procedure TfABMCategoriasVales.ActualizarClick(Sender: TObject);
var
  q: TFXQuery;
begin
  q := CrearQuery;
  try
    q.SQL.Text := 'Select * from CategoriasVales where 1=1';
...
procedure TfABMCategoriasVales.btnEliminarClick(Sender: TObject);
begin
  if cdsDatos.FieldbyName('Codigo').asInteger < 17 then
  begin
    MostrarDialogoAceptar('No es posible eliminar esta categoria. Es una categoria base.');
    Exit;
  end;

  if fNuevaCategoriaVale = nil then
    Application.CreateForm(TfNuevaCategoriaVale, fNuevaCategoriaVale)
  else
    fNuevaCategoriaVale.Show;

  fNuevaCategoriaVale.Cargar(cdsDatos.FieldbyName('Codigo').asInteger, toEliminar);
end;

procedure TfABMCategoriasVales.btnImprimirClick(Sender: TObject);
var
...
procedure TfABMCategoriasVales.btnModificarClick(Sender: TObject);
begin
//  if cdsDatos.FieldbyName('Codigo').asInteger < 17 then
//  begin
//    MostrarDialogoAceptar('No es posible modificar esta categoria. Es una categoria base.');
//    Exit;
//  end;
  ...
  fNuevaCategoriaVale.Cargar(cdsDatos.FieldbyName('Codigo').asInteger, toModificar);
end;

procedure TfABMCategoriasVales.btnNuevoClick(Sender: TObject);
begin
  if fNuevaCategoriaVale = nil then
    Application.CreateForm(TfNuevaCategoriaVale, fNuevaCategoriaVale)
  else
  ...
  fNuevaCategoriaVale.Cargar(0, toNuevo);
end;

procedure TfABMCategoriasVales.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TfABMCategoriasVales.edDescripcionChange(Sender: TObject);
begin
...
procedure TfABMCategoriasVales.edDesdeChange(Sender: TObject);
begin
...
procedure TfABMCategoriasVales.edHastaChange(Sender: TObject);
begin
...
procedure TfABMCategoriasVales.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  fABMCategoriasVales := nil;
  Action := caFree;
end;

procedure TfABMCategoriasVales.FormCreate(Sender: TObject);
begin
  fPrincipal.Center(Self);
  prPrepararDataset;
end;

procedure TfABMCategoriasVales.prPrepararDataset;
begin
...
function TfABMCategoriasVales.TituloGetFieldValue(aFieldName: string): Variant;
begin
...
procedure TfABMCategoriasVales.TodosCodigosClick(Sender: TObject);
begin
  if todosCodigos.Checked then
  begin
    edDesde.Text:='';
...
procedure TfABMCategoriasVales.TodosDescripcionClick(Sender: TObject);
begin
  if todosDescripcion.Checked then
    edDescripcion.Text:='';
