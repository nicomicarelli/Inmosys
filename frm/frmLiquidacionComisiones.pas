...
uses
  ...
  CheckLst,IBTable, IBCustomDataSet, FXQuery,
  ...
  dxSkinXmas2008Blue, cxTextEdit, cxMaskEdit, cxDropDownEdit, AdvObj, BaseGrid,
  AdvGrid, dxSkinBlueprint, dxSkinDevExpressDarkStyle, dxSkinDevExpressStyle,
  dxSkinHighContrast, dxSkinMetropolis, dxSkinMetropolisDark,
  dxSkinOffice2010Black, dxSkinOffice2010Blue, dxSkinOffice2010Silver,
  dxSkinOffice2013DarkGray, dxSkinOffice2013LightGray, dxSkinOffice2013White,
  dxSkinSevenClassic, dxSkinSharpPlus, dxSkinTheAsphaltWorld, dxSkinVS2010,
  dxSkinWhiteprint, ppDesignLayer, System.Actions, AdvGlowButton;

type
  TfLiquidacionComisiones = class(TForm)
...
var
  fLiquidacionComisiones: TfLiquidacionComisiones;

implementation

uses frmprincipal, frmConfirmacionRecibos, frmConfirmacionFacturas;
{$R *.DFM}

procedure TfLiquidacionComisiones.FormClose(Sender: TObject;
...
begin
  ...
  fLiquidacionComisiones := nil;
  Action := caFree;
end;

procedure TfLiquidacionComisiones.FormCreate(Sender: TObject);
var
  q: TFXQuery;
...
procedure TfLiquidacionComisiones.gListaGetAlignment(Sender: TObject; ARow,
...
function TfLiquidacionComisiones.plTituloGetFieldValue(aFieldName: string)
  : Variant;
begin
...
procedure TfLiquidacionComisiones.Edit1KeyPress(Sender: TObject; var Key: Char);
var
  Codigo:      string;
  I:           Integer;
  q: TFXQuery;
...
procedure TfLiquidacionComisiones.Edit6KeyPress(Sender: TObject; var Key: Char);
var
  Fecha: string;
...
procedure TfLiquidacionComisiones.Edit7KeyPress(Sender: TObject; var Key: Char);
var
  Fecha: string;
begin
  Fecha := Edit7.Text;
...
procedure TfLiquidacionComisiones.Edit8KeyPress(Sender: TObject; var Key: Char);
var
  Fecha: string;
  Dia: string;
  Mes: string;
...
procedure TfLiquidacionComisiones.edReferenteKeyPress(Sender: TObject;
  var Key: Char);
var
  I: Integer;
  q: TFXQuery;
  ...
  try
    ...
    begin
      ...
      begin
        Close;
        Sql.Clear;
        Sql.Add('Select * from inmuebles where Referente= :PROPIETARIO');
        ParamByName('Referente').AsString := Trim(edReferente.Text);
        Open;
        Edit1.Text := Fields[0].AsString;
        edit2.Text := Fields[1].AsString;
        Edit4.Text := Fields[4].AsString;
        ...
        begin
          ...
          begin
            ...
            gLista.AddCheckBox(0,I,false,false);
            gLista.Cells[1,I] := FieldByName('Codinq').AsString;
            gLista.Cells[2,I] := FieldByName('Ubicacion').AsString;
            gLista.Readonly[1,I] := True;
            gLista.Readonly[2,I] := True;
            Inc(I);
            Next;
...
procedure TfLiquidacionComisiones.Edit2KeyPress(Sender: TObject; var Key: Char);
var
 q: TFXQuery;
 Tecla: Char;
begin
...
procedure TfLiquidacionComisiones.Edit5KeyPress(Sender: TObject; var Key: Char);
var
  Codigo: string;
  Fecha:  string;
  Hoy:    string;
...
procedure TfLiquidacionComisiones.Edit5PropertiesChange(Sender: TObject);
var
  Codigo: string;
  Fecha:  string;
  Hoy:    string;
...
procedure TfLiquidacionComisiones.Edit10KeyPress
...
procedure TfLiquidacionComisiones.Edit4KeyPress(Sender: TObject; var Key: Char);
var
  I: Integer;
var
  q: TFXQuery;
...
procedure TfLiquidacionComisiones.BitBtn1Click(Sender: TObject);
begin
  gDetalle.Vaciar;
  actCalcularComisiones.Execute;
end;

procedure TfLiquidacionComisiones.BitBtn5Click(Sender: TObject);
var
  I: Integer;
...
procedure TfLiquidacionComisiones.btnGrabarClick(Sender: TObject);
var
  q: TFXQuery;
  I: Integer;
  stUbicacion: string;
...
procedure TfLiquidacionComisiones.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TfLiquidacionComisiones.actRecibosExecute(Sender: TObject);
var
  Fecha: string;
  Importe: string;
  q: TFXQuery;
...
procedure TfLiquidacionComisiones.actCalcularComisionesExecute(Sender: TObject);
var
  Propietario: string;
  Inq: string;
  pro: string;
  ...
  try
    try
      ...
      begin
        ...
        while not q.eof do
        begin
          if (q.Fieldbyname('Tributa').AsInteger = 1) and (q.Fieldbyname('CodItem').AsInteger = 121) then
          ...
          else if (q.Fieldbyname('Tributa').AsInteger = 0) then
            InsertarMasItem(MasItems, q.Fieldbyname('Codinq').AsString, q.Fieldbyname('Fecha')
              .AsString, q.Fieldbyname('Item').AsString, q.Fieldbyname('Periodo')
              .AsString, q.Fieldbyname('Importe').AsString, q.Fieldbyname
              ('CodPro').AsString, q.Fieldbyname('CodItem').AsString,
        ...
        begin
          ...
          begin
            ...
            begin
              Close;
              Sql.Clear;
              Sql.Add('select distinct * from inmuebles i where Trim(i.ubicacion) like :calleu');
      ...
      while not MasItemsp.eof do
      begin
        InsertarLiqPrOrig(LiqPrOrig, MasItemsp.Fields[0].AsString, edReferente.Text, Fecha,
