...
type
  TfABMGeneracionMovimientos = class(TForm)
...
var
  fABMGeneracionMovimientos: TfABMGeneracionMovimientos;

implementation

uses frmDatosFijos, frmPrincipal,
  frmNuevoImpuesto, ABMImpuestosYServicios, frmNuevoCedulon,
  ABMIdentificacionImpuestos, frmNuevoMovimientoImpuestos;

{$R *.dfm}

procedure TfABMGeneracionMovimientos.ActualizarClick(Sender: TObject);
var
  q: TFXQuery;
  I: Integer;
begin
...
function TfABMGeneracionMovimientos.TituloGetFieldValue(aFieldName: string): Variant;
begin
...
procedure TfABMGeneracionMovimientos.TodosCodigosClick(Sender: TObject);
begin
  if todosCodigos.Checked then
  begin
    edDesde.Text:='';
...
procedure TfABMGeneracionMovimientos.TodosDescripcionClick(Sender: TObject);
begin
  if todosDescripcion.Checked then
    edDescripcion.Text:='';
...
procedure TfABMGeneracionMovimientos.AdvGlowButton1Click(Sender: TObject);
begin
  gDetalle.SearchFooter.Visible := not gdetalle.SearchFooter.Visible;
end;

procedure TfABMGeneracionMovimientos.btnEliminarClick(Sender: TObject);
begin
...
procedure TfABMGeneracionMovimientos.btnImprimirClick(Sender: TObject);
begin
...
procedure TfABMGeneracionMovimientos.btnModificarClick(Sender: TObject);
begin
  if gDetalle.Cells[0,gDetalle.Row] = '' then
    Exit;
  ...
  if fNuevoMovimientoImpuestos = nil then
    Application.CreateForm(TfNuevoMovimientoImpuestos, fNuevoMovimientoImpuestos)
  else
    fNuevoMovimientoImpuestos.Show;
  fNuevoMovimientoImpuestos.edCodigo.Text  := gDetalle.Cells[0, gDetalle.Row];
  fNuevoMovimientoImpuestos.edEmpresa.Text := gDetalle.Cells[1, gDetalle.Row];
  fNuevoMovimientoImpuestos.edMes.Itemindex := fNuevoMovimientoImpuestos.edMes.Properties.Items.IndexOf(gDetalle.Cells[2, gDetalle.Row]);
  fNuevoMovimientoImpuestos.spAnio.Text    := gDetalle.Cells[3, gDetalle.Row];

  fNuevoMovimientoImpuestos.ActualizarClick(nil);

end;

procedure TfABMGeneracionMovimientos.btnNuevoClick(Sender: TObject);
begin
  if fAsignacionBoletas = nil then
    Application.CreateForm(TfAsignacionBoletas, fAsignacionBoletas)
  else
...
procedure TfABMGeneracionMovimientos.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TfABMGeneracionMovimientos.edDescripcionChange(Sender: TObject);
begin
...
procedure TfABMGeneracionMovimientos.edDesdeChange(Sender: TObject);
begin
...
procedure TfABMGeneracionMovimientos.edDesdeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  edHasta.Text := edDesde.text;
end;
...
procedure TfABMGeneracionMovimientos.edDesdeKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfABMGeneracionMovimientos.edHastaChange(Sender: TObject);
begin
...
procedure TfABMGeneracionMovimientos.fAsignacionBoletasNuevoCopiadelSeleccionado1Click(
  Sender: TObject);
begin
  if gDetalle.Cells[0,gDetalle.Row] = '' then
    Exit;
  ...
  fAsignacionBoletas.Cargar(gDetalle.Cells[0,gDetalle.Row],toModificar, gDetalle.Cells[2,gDetalle.Row], gDetalle.Cells[3,gDetalle.Row], gDetalle.Cells[4,gDetalle.Row]='SI', True);
end;

procedure TfABMGeneracionMovimientos.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action := caFree;
  fABMGeneracionMovimientos := nil;
end;

procedure TfABMGeneracionMovimientos.FormCreate(Sender: TObject);
begin
...
procedure TfABMGeneracionMovimientos.gDetalleFixedColClick(Sender: TObject; col: Integer);
begin
//  gDetalle.SortColumn(Col, True,'S');
end;

procedure TfABMGeneracionMovimientos.prPonerTitulosGrilla(pGrilla: TAdvStringGrid);
var
...
function TfABMGeneracionMovimientos.fxValidarPorcentajesPago: Boolean;
var
  q: TFXQuery;
begin
  Result := True;
