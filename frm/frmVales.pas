...
type
  ...
  TFVales = class(TForm)
    ...
    N2: TMenuItem;
    Cambiaritemal27: TMenuItem;
    CambiarItemal9: TMenuItem;
...
begin
  ...
  CambiarItemAl27.Visible := cdsDatos.FieldByName('Coditem').AsString = '09';
  CambiarItemAl9.Visible := cdsDatos.FieldByName('Coditem').AsString = '27';
...
//  edPendiente.Text := '$ ' + FormatFloat(',0.00',gDatos.DataController.Summary.FooterSummaryValues[0]);
  ...
  qCuerpo := CrearQuery;
  cdsAgrupado.DisableControls;
  try
    qCuerpo.SQL.Text :=
      ' Select Codinq, Sum(Entra - Sale) as Diferencia, Codigoitem from '+ TablaCuerpo +
      '  Where Codigo = :Codigo Group by 1,3';
    qCuerpo.ParamByName('Codigo').AsInteger := FCodigo;
    qCuerpo.Open;
      cdsAgrupado.EmptyDataset;
      while not qCuerpo.eof do
      begin
        if qCuerpo.FieldByName('Diferencia').AsFloat <= 0 then
        begin
          qCuerpo.Next;
          Continue;
        end;
        ...
        cdsAgrupado.Append;
//        cdsAgrupado.FieldByName('Seleccionado').AsBoolean := True;
        cdsAgrupado.FieldByName('Codigo').AsString := qCuerpo.FieldByName('Codinq').AsString;
        cdsAgrupado.FieldByName('Descripcion').AsString := edtDescripcion.Text;
//        cdsAgrupado.FieldByName('Linea').AsInteger := qCuerpo.FieldByName('Linea').AsInteger;
//        cdsAgrupado.FieldByName('Entra').AsFloat := qCuerpo.FieldByName('Entra').AsFloat;
//        cdsAgrupado.FieldByName('Sale').AsFloat := qCuerpo.FieldByName('Sale').AsFloat;
//        cdsAgrupado.FieldByName('Fecha').AsDatetime := qCuerpo.FieldByName('Fecha').AsDatetime;
        cdsAgrupado.FieldByName('Coditem').AsString := qCuerpo.FieldByName('Codigoitem').AsString;
//        cdsAgrupado.FieldByName('ID').AsString := qCuerpo.FieldByName('ID').AsString;
//        cdsAgrupado.FieldByName('Usuario').AsString := qCuerpo.FieldByName('Usuario').AsString;
        if qCuerpo.FieldByName('Diferencia').AsFloat > 0 then
          cdsAgrupado.FieldByName('Entra').AsFloat := qCuerpo.FieldByName('Diferencia').AsFloat;
        if qCuerpo.FieldByName('Diferencia').AsFloat < 0 then
          cdsAgrupado.FieldByName('Entra').AsFloat := qCuerpo.FieldByName('Diferencia').AsFloat;
//        cdsAgrupado.FieldByName('EsManual').AsBoolean := qCuerpo.FieldByName('EsManual').AsInteger = 1;
//        cdsAgrupado.FieldByName('PasadoACaja').AsBoolean := qCuerpo.FieldByName('PasadoACaja').AsInteger = 1;
        cdsAgrupado.Post;
...
procedure TFVales.Cambiaritemal27Click(Sender: TObject);
var
  q: TFXQuery;
...
begin
  ...
  try
    try
      Operacion := toNuevo;
      stDescripcion := cdsDatos.FieldByName('Descripcion').AsString;
      stDescripcion := AnsiReplaceStr(stDescripcion, 'IMP Y/O SERVICIOS', 'IMPUESTOS A DESCONTAR');
      q.SQL.Text := ' Update CuerpoVales Set Codigoitem = ''27'', Descripcion = :Descripcion Where Codigo = :COdigoVale and Linea = :Linea ';
      q.ParamByName('CodigoVale').AsString := edtCodigo.Text;
      q.ParamByName('Linea').AsInteger := cdsDatos.FieldByName('Linea').AsInteger;
      q.ParamByName('Descripcion').AsString := stDescripcion;
      q.ExecSql;

      DM.ConfirmarTransaccion;
      MostrarDialogoAceptar('Información','La modificacion del codigo 09 al 27 se grabo satisfactoriamente', mtInformation);
    except
      DM.CancelarTransaccion;
    end;
  finally
...
procedure TFVales.CambiarItemal9Click(Sender: TObject);
var
  q: TFXQuery;
  stDescripcion: string;
...
begin
  ...
  try
    try
      Operacion := toNuevo;
      stDescripcion := cdsDatos.FieldByName('Descripcion').AsString;
      stDescripcion := AnsiReplaceStr(stDescripcion, 'IMPUESTOS A DESCONTAR', 'IMP Y/O SERVICIOS');
      q.SQL.Text := ' Update CuerpoVales Set Codigoitem = ''09'', Descripcion = :Descripcion Where Codigo = :COdigoVale and Linea = :Linea ';
      q.ParamByName('CodigoVale').AsString := edtCodigo.Text;
      q.ParamByName('Linea').AsInteger := cdsDatos.FieldByName('Linea').AsInteger;
      q.ParamByName('Descripcion').AsString := stDescripcion;
      q.ExecSql;
      ...
      DM.ConfirmarTransaccion;
      MostrarDialogoAceptar('Información','La modificacion del codigo 27 al 09 se grabo satisfactoriamente', mtInformation);
    except
      DM.CancelarTransaccion;
    end;
  finally
