...
uses
  ...
  Numedit, FXQuery, System.Actions, System.UITypes, AdvGlowButton;

type
  TfMovimientosCtaCte = class(TForm)
  ...
  public
    ...
    procedure Cargar(pCodigo: string; Operacion: TTipoOperacion; pIDMovimiento: string='');
    function ValidarDatos: Boolean;
  end;

var
  fMovimientosCtaCte: TfMovimientosCtaCte;

implementation

uses frmDatos, frmprincipal, frmABMDatosFijos, frmABMRazonesSociales,
  frmABMCtaCteInquilinos, frmABMMovimientos;

{$R *.DFM}

procedure TfMovimientosCtaCte.FormActivate(Sender: TObject);
var
  q: tFXQuery;
begin
  q := CrearQuery;
...
procedure TfMovimientosCtaCte.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  FMovimientosCtacte := nil;
end;

procedure TfMovimientosCtaCte.FormCreate(Sender: TObject);
begin
  fPrincipal.Center(Self);
end;

function TfMovimientosCtaCte.ValidarDatos: Boolean;
begin
  Result := True;
...
procedure TfMovimientosCtaCte.edtDescripcionKeyPress(Sender: TObject; var Key: Char);
begin
 if key=#13 then
   if Trim(edtDescripcion.Text) = '' then
...
procedure TfMovimientosCtaCte.edtFechaExit(Sender: TObject);
begin
  try
...
procedure TfMovimientosCtaCte.edtFechaKeyPress(Sender: TObject; var Key: Char);
var
  Fecha: string;
begin
  Fecha := EdtFecha.Text;
...
procedure TfMovimientosCtaCte.edtSobreDestinoKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfMovimientosCtaCte.actNuevoExecute(Sender: TObject);
var
  I: Integer;
...
procedure TfMovimientosCtaCte.actGrabarExecute(Sender: TObject);
var
  q: tFXQuery;
  Propietario: string;
begin
...
//      InsertarItemCtaCteInquilino(edtCodinq.text, Date, Date, cbxDatoFijo.Text,EdtDescripcion.Text, FormatFloat('0.00', ToFloat(edtImporte.Text)), Propietario, edtCodigo.Text,3, -1, rdbEntra.Checked)
    else
    begin
      q.sql.text :=
        ' UPDATE CTACTEINQUILINO '+
        '    SET CODINQ = :Codinq, '+
        '        FECHA = :Fecha, '+
        '        ITEM = :Item, '+
        '        PERIODO = :Periodo, '+
...
procedure TfMovimientosCtaCte.actBorrarExecute(Sender: TObject);
var
  q: tFXQuery;

begin
  if not MostrarDialogoSino('Inmosys', '¿Esta seguro de elimiar el elemento seleccionado?', mtConfirmation) then
    Exit;
  ...
  try
    try
      q.SQL.Text := ' Delete from CtaCteInquilino where Registro = :Registro';
      q.ParamByName('Registro').AsInteger := ToInt(edtRegistro.Text);
      q.ExecSQL();

      q.SQL.Text := ' Delete from Itemautom where Registro = :Registro';
      q.ParamByName('Registro').AsInteger := ToInt(edtIDMovimiento.Text);
      q.ExecSQL();
      DM.ConfirmarTransaccion;
      MostrarDialogoAceptar( 'Datos eliminados correctamente.');
      Close;
      if Assigned(fAbMCtaCteInquilinos) then
      ...
      MostrarDialogoAceptar( 'Ocurrio un error al eliminar el dato Fijo.', mtError);
    end;
  finally
    FreeAndNil(q);
  end;
...
procedure TfMovimientosCtaCte.DatosKeyPress(Sender: TObject; var Key: Char);
var
  q: tFXQuery;
begin
  q := CrearQuery;
...
procedure TfMovimientosCtaCte.edtCodigoKeyPress(Sender: TObject; var Key: Char);
var
  Coditem: string;
  Nro:     string;
  q: tFXQuery;
...
procedure TfMovimientosCtaCte.edtCodinqEnter(Sender: TObject);
begin
...
procedure TfMovimientosCtaCte.edtCodinqKeyPress(Sender: TObject; var Key: Char);
var
  q: tFXQuery;
...
procedure TfMovimientosCtaCte.actBorrarUpdate(Sender: TObject);
begin
  actBorrar.Enabled := (Trim(EdtCodigo.Text) <> '');
end;

procedure TfMovimientosCtaCte.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TfMovimientosCtaCte.Cargar(pCodigo: string; Operacion: TTipoOperacion; pIDMovimiento: string='');
var
  q: tFXQuery;
  Tecla: Char;
begin
  ...
  begin
    ...
    try
      q.SQL.Text :=
        ' Select C.Fecha, C.Codinq, I.Inquilino, C.Coditem, C.Item, C.Periodo, C.Debe, C.Haber, C.Registro, C.IDMovimiento ' +
      ...
      edtIDMovimiento.Text := q.FieldByName('IDMovimiento').AsString;
      Tecla := #13;
    ...
    begin
      actBorrar.Execute;
      if Assigned(fMovimientosCtaCte) then
        Close;
    end;
  end;
end;

procedure TfMovimientosCtaCte.cbxDatoFijoClick(Sender: TObject);
var
  Codigo: string;
  Fecha:  string;
  Numero: string;
