...
type
  TfABMImpuestosPagados = class(TForm)
...
var
  fABMImpuestosPagados: TfABMImpuestosPagados;
  fABMDevolucionDinero: TfABMImpuestosPagados;
...
uses frmDatosFijos, frmPrincipal,
  ...
  frmNuevoCedulonPagado, frmNuevaDevolucionDinero, ListadoImpuestosYServicios;

{$R *.dfm}

procedure TfABMImpuestosPagados.ActualizarClick(Sender: TObject);
var
  q: TFXQuery;
  I: Integer;
begin
  ...
  try
    q.SQL.Text :=
      ' Select Distinct C.*, E.Descripcion '+
...
function TfABMImpuestosPagados.TituloGetFieldValue(aFieldName: string): Variant;
begin
...
procedure TfABMImpuestosPagados.TodosCodigosClick(Sender: TObject);
begin
  if todosCodigos.Checked then
  begin
    edDesde.Text:='';
...
procedure TfABMImpuestosPagados.TodosDescripcionClick(Sender: TObject);
begin
  if todosDescripcion.Checked then
    edDescripcion.Text:='';
...
procedure TfABMImpuestosPagados.AdvGlowButton1Click(Sender: TObject);
begin
  gDetalle.SearchFooter.Visible := not gdetalle.SearchFooter.Visible;
end;

procedure TfABMImpuestosPagados.btnEliminarClick(Sender: TObject);
begin
...
procedure TfABMImpuestosPagados.btnImprimirClick(Sender: TObject);
begin
...
procedure TfABMImpuestosPagados.btnModificarClick(Sender: TObject);
begin
  if gDetalle.Cells[0,gDetalle.Row] = '' then
    Exit;
  ...
  if PantallaActiva = paImpuestosPagados then
  begin
    ...
    fNuevoCedulonPagado.Cargar(gDetalle.Cells[0,gDetalle.Row],toModificar,
      gDetalle.Cells[2,gDetalle.Row], gDetalle.Cells[3,gDetalle.Row],
      gDetalle.Cells[4,gDetalle.Row]);
  end
  else
  begin
    if fNuevaDevolucionDinero = nil then
      Application.CreateForm(TfNuevaDevolucionDinero, fNuevaDevolucionDinero)
    else
      fNuevaDevolucionDinero.Show;

    fNuevaDevolucionDinero.Cargar(gDetalle.Cells[0,gDetalle.Row],toModificar,
      gDetalle.Cells[2,gDetalle.Row], gDetalle.Cells[3,gDetalle.Row],
      gDetalle.Cells[4,gDetalle.Row]);
  end;
end;

procedure TfABMImpuestosPagados.btnNuevoClick(Sender: TObject);
begin
  if gDetalle.Cells[0,gDetalle.Row] = '' then
    Exit;
...
procedure TfABMImpuestosPagados.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TfABMImpuestosPagados.edDescripcionChange(Sender: TObject);
begin
...
procedure TfABMImpuestosPagados.edDesdeChange(Sender: TObject);
begin
...
procedure TfABMImpuestosPagados.edDesdeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  edHasta.Text := edDesde.text;
end;
...
procedure TfABMImpuestosPagados.edDesdeKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfABMImpuestosPagados.edHastaChange(Sender: TObject);
begin
...
procedure TfABMImpuestosPagados.fAsignacionBoletasNuevoCopiadelSeleccionado1Click(
  Sender: TObject);
begin
  if gDetalle.Cells[0,gDetalle.Row] = '' then
    Exit;
...
procedure TfABMImpuestosPagados.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action := caFree;
  if Assigned(fABMImpuestosPagados) then
    fABMImpuestosPagados := nil;
  if Assigned(fABMDevolucionDinero) then
    fABMDevolucionDinero := nil;

end;

procedure TfABMImpuestosPagados.FormCreate(Sender: TObject);
begin
...
procedure TfABMImpuestosPagados.gDetalleFixedColClick(Sender: TObject; col: Integer);
begin
//  gDetalle.SortColumn(Col, True,'S');
end;

procedure TfABMImpuestosPagados.prPonerTitulosGrilla(pGrilla: TAdvStringGrid);
var
...
function TfABMImpuestosPagados.fxValidarPorcentajesPago: Boolean;
var
  q: TFXQuery;
begin
  Result := True;
