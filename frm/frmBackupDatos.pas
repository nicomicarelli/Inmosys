...
type
  TfBackupDatos = class(TForm)
    lblCodigo: TLabel;
    edRuta: TEdit;
    Carpeta: TCheckDirectoryListBoxEx;
    DiscoExterno: TCheckDirectoryListBoxEx;
...
procedure TfBackupDatos.DelFilesFromDir(Directory, FileMask: string; DelSubDirs: Boolean);
var
  SourceLst: string;
  FOS: TSHFileOpStruct;
begin
...
procedure TfBackupDatos.DiscoClick(Sender: TObject);
begin
  DiscoExterno.Refresh
end;

procedure TfBackupDatos.CopyFilesToPath(aFiles: array of string; DestPath: string);
var
...
procedure TfBackupDatos.AdvGlowButton2Click(Sender: TObject);
var
...
procedure TfBackupDatos.btnEliminarClick(Sender: TObject);
var
  I: Integer;
  Ruta: string;
begin
...
procedure TfBackupDatos.btnGrabarClick(Sender: TObject);
begin
  if MostrarDialogoSiNO('Se generará el backup de los datos del sistemsa. ¿Desea Continuar? ', mbYes) then
  begin
...
procedure TfBackupDatos.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TfBackupDatos.CarpetaClickCheck(Sender: TObject; Index: Integer);
var
  Ruta: string;
begin
  Carpeta.Refresh;
...
procedure TfBackupDatos.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  fBackUpDatos := nil;
end;

procedure TfBackupDatos.FormCreate(Sender: TObject);
begin
