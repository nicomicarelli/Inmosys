unit frmHistorialRetiroLiquidaciones;
...
type
  TfHistorialRetiroLiquidaciones = class(TForm)
...
var
  fHistorialRetiroLiquidaciones: TfHistorialRetiroLiquidaciones;
...
procedure TfHistorialRetiroLiquidaciones.ActualizarClick(Sender: TObject);
var
  q: TFXQuery;
...
procedure TfHistorialRetiroLiquidaciones.TodosFPagoClick(Sender: TObject);
begin
...
function TfHistorialRetiroLiquidaciones.TituloGetFieldValue(
...
procedure TfHistorialRetiroLiquidaciones.TodosCodigosClick(Sender: TObject);
begin
  if todosCodigos.Checked then
  begin
    edDesde.Text:='';
...
procedure TfHistorialRetiroLiquidaciones.VerContratoEscalonado1Click(Sender: TObject);
var
  Tecla: Char;
begin
...
procedure TfHistorialRetiroLiquidaciones.btnEliminarClick(Sender: TObject);
begin
...
procedure TfHistorialRetiroLiquidaciones.btnGrabarClick(Sender: TObject);
begin
    ImprimirReporte ( Reporte,  plReporte,  gDetalle ) ;
end;

procedure TfHistorialRetiroLiquidaciones.btnModificarClick(Sender: TObject);
begin
  if gDetalle.Cells[6,gDetalle.Row] = '' then
    Exit;
...
procedure TfHistorialRetiroLiquidaciones.btnNuevoClick(Sender: TObject);
begin
  if FMovimientosCtaCte = nil then
    Application.CreateForm(TFMovimientosCtaCte, FMovimientosCtaCte)
  else
...
procedure TfHistorialRetiroLiquidaciones.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TfHistorialRetiroLiquidaciones.cbFechaPagoClick(Sender: TObject);
begin
  todosFPago.Checked := False;
end;

procedure TfHistorialRetiroLiquidaciones.edDescripcionKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
    edtFecha.SetFocus;
...
procedure TfHistorialRetiroLiquidaciones.edDesdeChange(Sender: TObject);
begin
...
procedure TfHistorialRetiroLiquidaciones.edDesdeKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfHistorialRetiroLiquidaciones.edFechaHastaChange(Sender: TObject);
begin
//  TodosFPago.checked := Trim(edFechaHasta.Text) = '';
end;

procedure TfHistorialRetiroLiquidaciones.edFechaHastaKeyPress(Sender: TObject; var Key: Char);
var
  Dia:   string;
  Fecha: string;
...
procedure TfHistorialRetiroLiquidaciones.edHastaChange(Sender: TObject);
begin
...
procedure TfHistorialRetiroLiquidaciones.edtFechaChange(Sender: TObject);
begin
  TodosFPago.checked := Trim(edtFecha.Text) = '';
//  edFechahasta.Text:=edtfecha.Text;
end;

procedure TfHistorialRetiroLiquidaciones.edtFechaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  edFechaHasta.Text := edtfecha.text;
end;

procedure TfHistorialRetiroLiquidaciones.edtFechaKeyPress(Sender: TObject; var Key: Char);
var
  Dia:   string;
  Fecha: string;
...
procedure TfHistorialRetiroLiquidaciones.edtItemDesdeChange(Sender: TObject);
begin
//  edItemhasta.Text:=edtItemdesde.Text;

end;

procedure TfHistorialRetiroLiquidaciones.edtItemDesdeKeyPress(Sender: TObject; var Key: Char);
begin
  if  not (CharInSet(Key, ['0'..'9', #8, #13])) then
    Key := #0;
...
procedure TfHistorialRetiroLiquidaciones.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  action := caFree;
  fHistorialRetiroLiquidaciones := nil;
end;

procedure TfHistorialRetiroLiquidaciones.FormCreate(Sender: TObject);
begin
  fPrincipal.Center(Self);
end;

procedure TfHistorialRetiroLiquidaciones.gDetalleDblClick(Sender: TObject);
begin
...
procedure TfHistorialRetiroLiquidaciones.gDetalleFixedColClick(Sender: TObject; col: Integer);
begin
  if Col = 2 then
