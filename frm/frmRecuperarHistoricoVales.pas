unit frmRecuperarHistoricoVales;
...
uses
  ...
  Vcl.Menus, Provider, frmDatos;

type
  TFRecuperarHistoricoVales = class(TForm)
...
var
  FRecuperarHistoricoVales: TFRecuperarHistoricoVales;
...
procedure TFRecuperarHistoricoVales.ActualizarClick(Sender: TObject);
var
  q: TFXQuery;
begin
  q := CrearQuery;
  try
    ...
    '                from HistoricoCabezaVales CA ' +
    '               Inner Join CategoriasVales V on V.Codigo = CA.CodigoCategoria '+
    '               Inner Join HistoricoCuerpoVales CU on CA.Codigo = Cu.Codigo '+
    ' where 1=1 ' ;
    ...
    q.SQL.Add(' order by CA.Codigo ');
    if cbCategoria.ItemIndex > 0 then
      q.ParambyName('Categoria').AsInteger := cbCategoria.ItemIndex;
    q.Open;
...
procedure TFRecuperarHistoricoVales.btnEliminarClick(Sender: TObject);
begin
  if FVales = nil then
...
procedure TFRecuperarHistoricoVales.btnImprimirClick(Sender: TObject);
var
  q, q1: TFXQuery;
  I: Integer;
  CDS: TClientDataset;
  ...
  try
    try
      ...
      begin
        ...
        q.SQL.Text := ' Insert into CabezaVales '+
                      ' Select C.* From HistoricoCabezaVales C ';
        q.SQL.Add(    '  Where C.Codigo = :InqDesde');
        q.SQL.Add(    '    and not exists (Select Codigo from cabezavales h1 where h1.codigo = c.codigo)');
        q.ParambyName('InqDesde').AsInteger := cdsDatos.FieldByName('Codigo').AsInteger;
        q.ExecSql;

        q1.SQL.Text := 'Insert into Cuerpovales  '+
                      ' Select * From historicoCuerpoVales C ';
        q1.SQL.Add(   '  Where Codigo = :InqDesde ');
        q1.ParambyName('InqDesde').AsInteger :=cdsDatos.FieldByName('Codigo').AsInteger;
        q1.ExecSql;


        q1.SQL.Text := ' Select C.Codigo From historicoCabezaVales C ';
        ...
        while not q1.eof do
        begin
          q.SQL.Text := ' delete From historicoCabezavales Where Codigo = :ID';
          q.ParambyName('ID').AsInteger := ToInt(q1.FieldByName('Codigo').AsString);
          q.ExecSql;

          q.SQL.Text := ' delete From historicoCuerpoVales Where Codigo = :ID';
          q.ParambyName('ID').AsInteger := ToInt(q1.FieldByName('Codigo').AsString);
          q.ExecSql;
...
procedure TFRecuperarHistoricoVales.btnModificarClick(Sender: TObject);
begin
  if FVales = nil then
    Application.CreateForm(TFVales, FVales)
  else
...
procedure TFRecuperarHistoricoVales.btnNuevoClick(Sender: TObject);
begin
...
procedure TFRecuperarHistoricoVales.btnSalirClick(Sender: TObject);
begin
  Close;
end;

procedure TFRecuperarHistoricoVales.chTodosCodinqClick(Sender: TObject);
begin
...
procedure TFRecuperarHistoricoVales.Deseleccionartodos1Click(Sender: TObject);
begin
  cdsDatos.First;
  while not cdsDatos.eof do
  begin
...
procedure TFRecuperarHistoricoVales.edDescripcionChange(Sender: TObject);
begin
...
procedure TFRecuperarHistoricoVales.edDesdeChange(Sender: TObject);
begin
...
procedure TFRecuperarHistoricoVales.edHastaChange(Sender: TObject);
begin
...
procedure TFRecuperarHistoricoVales.edCodinqDesdeChange(Sender: TObject);
begin
...
procedure TFRecuperarHistoricoVales.edCodinqHastaChange(Sender: TObject);
begin
...
procedure TFRecuperarHistoricoVales.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  if Assigned(fRecuperarHistoricoVales) then
    fRecuperarHistoricoVales := nil;
end;

procedure TFRecuperarHistoricoVales.FormCreate(Sender: TObject);
begin
  ...
  qCategorias.sql.text := 'Select 0 as Codigo, '''' as Descripcion from CategoriasVales union Select Codigo, descripcion from CategoriasVales';
  qCategorias.Open;
...
procedure TFRecuperarHistoricoVales.gDatosCustomDrawCell(Sender: TcxCustomGridTableView;
  ACanvas: TcxCanvas; AViewInfo: TcxGridTableDataCellViewInfo;
  var ADone: Boolean);
begin
...
procedure TFRecuperarHistoricoVales.GenerarMovimientoenlaCaja1Click(Sender: TObject);
begin
...
procedure TFRecuperarHistoricoVales.NuevoValecopiaFIELdelSeleccionado1Click(Sender: TObject);
begin
  if cdsDatos.IsEmpty then
    Exit;
...
procedure TFRecuperarHistoricoVales.prActualizar;
begin
  ActualizarClick(nil);
end;

procedure TFRecuperarHistoricoVales.prPrepararDataset;
begin
...
procedure TFRecuperarHistoricoVales.Seleccionartodos1Click(Sender: TObject);
begin
  cdsDatos.First;
  while not cdsDatos.eof do
  begin
...
function TFRecuperarHistoricoVales.TituloGetFieldValue(aFieldName: string): Variant;
begin
...
procedure TFRecuperarHistoricoVales.TodosCodigosClick(Sender: TObject);
begin
  if todosCodigos.Checked then
  begin
    edDesde.Text:='';
...
procedure TFRecuperarHistoricoVales.TodosDescripcionClick(Sender: TObject);
begin
  if todosDescripcion.Checked then
    edDescripcion.Text:='';
